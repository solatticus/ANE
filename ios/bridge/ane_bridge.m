// ane_bridge.m — Objective-C bridge to ANE private APIs
// Wraps _ANEInMemoryModel private APIs into C-callable functions
// Supports macOS and iOS (classes are in dyld shared cache on both)
...
int ane_bridge_init(void) {
    ...
    // Try explicit framework path first (macOS), then NULL handle (iOS shared cache)
    void *handle = dlopen(
        "/System/Library/PrivateFrameworks/AppleNeuralEngine.framework/AppleNeuralEngine",
        RTLD_NOW);
    if (!handle) {
        // On iOS, the framework is in the dyld shared cache — resolve from main image
