// train_lora.m — LoRA fine-tuning for stories110M on ANE
// Freezes all pretrained weights, trains only low-rank adapters on Wq and Wv.
// Uses CPU-side merge: W_eff = W_frozen + (alpha/rank) * B @ A before ANE compile.
// No changes to MIL generators or ANE kernel pipeline.
...
// ===== Weight loading from llama2.c format (same as train_large.m) =====
...
// ===== Kernel compilation (same as train_large.m) =====
...
int main(int argc, char *argv[]) {
    @autoreleasepool {
        ...
        // Gradient accumulators for frozen weights (written by backward, not used for Adam)
        ...
        // ── Load pretrained model (required) ──
        printf("=== ANE LoRA Fine-Tuning: Stories110M ===\n");
        ...
        if (do_resume) {
            ...
            if (resuming) {
                // Command-line args override checkpoint values
                if (user_steps > 0) total_steps = user_steps;
                if (user_lr > 0) lr = user_lr;
        ...
        printf("Accum %d steps per recompile | Adam LR=%.1e\n\n", ACCUM_STEPS, lr);
        ...
        while (step < total_steps) {
            // Check compile budget → save LoRA checkpoint and exec restart
            ...
            // ── Accumulation steps (forward + backward identical to train_large.m) ──
            ...
            // Scale gradients for Wq and Wv (used by LoRA gradient extraction)
            float gsc = 1.0f / steps_batch;
            for (int L=0; L<NLAYERS; L++) {
                for(size_t i=0;i<WQ_SZ;i++) { grads[L].Wq[i]*=gsc; grads[L].Wv[i]*=gsc; }
            }

            // ── Extract LoRA gradients and update ──
            adam_t++;
            for (int L=0; L<NLAYERS; L++) {
            ...
            // No Adam update on frozen weights (Wk, Wo, W1, W2, W3, rms, embed)

            // LoRA parameter norms (sum over all layers)
            ...
            fprintf(stderr, "{\"type\":\"lora_batch\",\"batch\":%d,\"avg_loss\":%.6f,\"norm_A\":%.6f,\"norm_B\":%.6f,"
